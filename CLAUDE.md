...
**A row of plates sharing one printed caption is comma-separated in one token:** `[[img:a.png,b.png|caption]]`. 12 of the book's 35 multi-image runs share a caption, so a separate block type would be a second rule to remember for a case that is one character away from the first. On the node it becomes `raw_params.images` + `image_sha1s` (with `image`/`image_sha1` still set to the first, so anything reading the single-file fields keeps working) and the reader lays them out in a row with the caption once underneath. MinerU does **not** auto-group them — it reports one block per image and the grouping is editorial judgement, which belongs in the override layer, not in the transcription.
