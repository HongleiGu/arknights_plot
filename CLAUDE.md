...
2j. `ocr_book.py` / `structure_book.py` / `import_book.py` — **standalone** (only the import is a pipeline step, alias `book`). The 大地巡旅 官方世界观设定集 scan → `stories` (category **设定集**) + `text_clusters` + `text_chunks` (008, no migration). The scan has **no text layer** (`pdftotext` over 5 pages returns 5 bytes), so OCR is unavoidable: RapidOCR (ONNX, ~50MB, no PaddlePaddle) at ~29s/page → **453 pages in 3.6h**, mean confidence 0.983, 1.5% of lines under 0.8. Result: **414,328 chars**, which matches the book's stated 40余万字 — so effectively the whole book. Resumable (one JSON per page; an existing page is skipped).
Recognition was never the hard part — **reading order** was. RapidOCR sorts boxes top-to-bottom with no layout model, so on a two-column page the right-hand caption is spliced into the middle of a left-hand paragraph: every character correct, meaning destroyed. `order_blocks()` is a lightweight XY-cut over the boxes RapidOCR already returns. Its gutter detector counts *coverage* rather than testing for strictly-empty columns, because one handwritten annotation running across the gutter otherwise erases a gap the other 40 lines agree on (`BRIDGE_TOL`).
`structure_book.py` rebuilds the outline from the book's own printed `CHAPTER n.m` markers (57 of them) → **61 sections**, 7 orphan front-matter pages. Four sections carry **no printed marker** (`EXTRA_SECTIONS`) — found by looking for runs of text pages with no rotated chapter tab; without them 6.8 (normally ~4k chars) silently swallowed 16 pages and reported 18k. `text_clusters.level_code` keeps the section number (`5.10.1`, `6.Extra`) because 泰拉年表 cites the book that way, and one chunk per **printed page** keeps the page number, which is how a claim gets checked against a physical copy.
Still open: character cleanup. 凯尔希's handwritten 批注 come back recognisable but wrong on proper nouns (为维多利亚辩护 → 为缠多利业辨辩护); that is an LLM pass with `entities` (026) as a 6885-name gazetteer, kept separate so re-running the cheap deterministic half never re-spends tokens. Also: the book is a commercial publication (ISBN 978-7-5514-4726-3) — importing makes it AI-searchable, it does **not** put it on a public page; that is a frontend decision and is deliberately unmade.
