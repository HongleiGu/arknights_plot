...
2j. `ocr_book.py` / `structure_book.py` / `import_book.py` — **standalone** (only the import is a pipeline step, alias `book`). The 大地巡旅 官方世界观设定集 scan → category **大地巡旅**, with **no new schema**: book chapter → `stories` (6 numbered + a synthetic 附录), section → `chapters`, paragraph → `nodes` (`type='subtitle'`, `speaker='narrator'`), illustration → `nodes` (`type='cgitem'`, AP-34).
**Why not 008's `text_clusters`/`text_chunks`, which is nominally the right shape:** the alternative needs a migration and this doesn't. Commenting on a paragraph would require `comment_anchors` to gain `text_chunk_id`, whereas `chapter_id`/`node_id` are already there — and reusing stories/chapters/nodes also inherits the chapter reader, `@chapter/N` + `@node/N` citations and `CiteSearch` for free. 008 is for short adjuncts appended to a chapter; this is 6×61×441 pages of standalone hierarchical prose, and hierarchy+length is what stories/chapters model. **The cost, which is real:** `nodes` is AVG-script shaped (`scene_id`, `branch_id`, `no_decision_in_branch`, `speaker` are all meaningless here) and this is its first non-script tenant, so ~5.8k rows are narration rather than dialogue. The only thing that reads `nodes.speaker` is `seed_entities.py`, which already excludes `narrator`. `raw_params.page` carries the printed page number — `nodes.seq` is paragraph order, so without it the audit trail back to a physical copy is lost.
OCR: the scan has **no text layer** (`pdftotext` over 5 pages returns 5 bytes). RapidOCR (ONNX, ~50MB, no PaddlePaddle) at ~29s/page → **453 pages in 3.6h**, mean confidence 0.983. Result **396,784 chars** against the book's stated 40余万字 — effectively the whole text. Resumable (one JSON per page).
Recognition was never the hard part; **reading order** was. RapidOCR sorts boxes top-to-bottom with no layout model, so a two-column page splices the right-hand caption into the middle of a left-hand paragraph — every character correct, meaning destroyed. `order_blocks()` is a lightweight XY-cut over the boxes RapidOCR already returns; its gutter detector counts *coverage* rather than requiring strictly-empty columns, because one handwritten annotation crossing the gutter otherwise erases a gap 40 other lines agree on (`BRIDGE_TOL`).
**Paragraphs are rebuilt from terminal punctuation, not the printed indent.** The indent is the obvious signal and it does not work here: the body's two-character indent lands at almost exactly the left edge of the indented pull-quote boxes, so an identical x0 means "new paragraph" in one place and "continuation" in another. A justified line breaking mid-sentence always ends on no punctuation. Geometry is kept only for hard breaks (a large horizontal jump = different column/text box). 18,363 OCR lines → 5,781 paragraphs.
`structure_book.py` rebuilds the outline from the book's own printed `CHAPTER n.m` markers (57) plus the contents spread (6 top-level chapters) → **61 sections**, 7 orphan front-matter pages. Four sections carry **no printed marker** (`EXTRA_SECTIONS`) — found by looking for runs of text pages with no rotated chapter tab; without them 6.8 (normally ~4k chars) silently swallowed 16 pages and reported 18k. Section numbers are kept in `chapters.level_code` because 泰拉年表 cites the book that way (`大地巡旅：6.Extra 罗德岛`).
Still open: character cleanup. **Confidence is the wrong flag for it** — the 267 lines under 0.8 confidence hold only 1,013 characters total (1–3 char folio fragments and stray marks), while the real errors (`为维多利亚辩护` → `为缠多利业辨辩护`, in 凯尔希's handwritten 批注) came back *confidently wrong*. An external gazetteer fails too: `entities` is character-only (`seed_entities.py` reads `nodes.speaker`), so the book's subject matter — 维多利亚, 乌萨斯, 谢拉格, 卡兹戴尔 — isn't in it, giving 18,135 false positives. The workable detector is **self-consistency**: 维多利亚 appears 476×, 乌萨斯 349×, so a rare near-variant of a frequent term is the error.
