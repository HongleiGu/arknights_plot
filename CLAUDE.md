...
**Adjacency is the definition of a block.** The editor format carries no block id, and two blocks written back to back are indistinguishable from one block of two paragraphs, so merging consecutive asides is the only consistent reading. A printed-page start also ends a run — the scan toggle it renders is full width, and the block genuinely continues on a new page. No run crosses a page boundary today (measured), so that condition is defensive.
